...
class ProductOfferDispatcher(OfferDispatcher):
    ...
    def __get_best_offer_for_a_bill_entry(self, bill_entry: BillEntry):
        product_offers = filter(lambda e: e.product_id ==
                                bill_entry.product_id, self.offers)
        best_offer = None
        best_discount = 0
    ...
    def __apply_best_offer_for_a_bill_entry(self, bill_entry):
        offer: ProductOffer = self.__get_best_offer_for_a_bill_entry(
            bill_entry)
        if offer:
            bill_entry.offer_id = offer.id
            bill_entry.net_price -= (bill_entry.net_price *
